...
vector<DataPoint> CsvDataHandler::read(const char* path, char delimiter)
{
	...
	if (!inputFile.is_open()) throw std::runtime_error("Could not open read csv file");
...
// writes a subscale table to a file at the given path
void CsvDataHandler::writeTable(const char* path, SubscaleTable* table)
{
	// Create an output filestream object
	std::ofstream myFile(path);
...
unsigned int CsvDataHandler::readTable(const char* path, SubspaceTable* table)
{
	...
	// Make sure the file is open
	if (!inputFile.is_open()) throw std::runtime_error("Could not open read Table file");
